  ...
  return (
    <div className="label-select" ref={ref}>
      <button type="button" className="label-select__trigger" onClick={() => setOpen((o) => !o)}>
        ...
        {selected.map((l) => (
          <span key={l} className="label-chip label-chip--removable" style={{ background: LABEL_COLORS[l].bg, color: LABEL_COLORS[l].text }}>
            {l}
            <button type="button" className="label-chip__remove" style={{ color: LABEL_COLORS[l].text }} onMouseDown={(e) => { e.stopPropagation(); toggle(l); }} aria-label={`Remove ${l}`}>✕</button>
          </span>
        ))}
      </button>
      {open && (
        <div className="label-select__dropdown">
          {ALL_LABELS.map((label) => {
            ...
            return (
              <button key={label} type="button" className={`label-select__option ${isSelected ? 'label-select__option--selected' : ''}`} onClick={() => toggle(label)}>
                <span className="label-select__check">{isSelected ? '✓' : ''}</span>
                <span className="label-chip" style={{ background: LABEL_COLORS[label].bg, color: LABEL_COLORS[label].text }}>{label}</span>
              </button>
            );
          })}
        </div>
...
function buildBreadcrumb(ticket: Ticket, allTickets: Ticket[]): Ticket[] {
  const chain: Ticket[] = [];
...
export function TicketDetail() {
  ...
  const { tickets, sprints, updateTicket, setTickets } = useTickets();
    ...
    return (
      <div className="td-page">
        <div className="td-page__not-found">
          ...
          <p>The ticket <code>{ticketId}</code> does not exist in this space.</p>
          <Link to="/board" className="td-page__back-link">Back to Board</Link>
        </div>
      </div>
    );
  }
  ...
  function saveAndNavigate(path: string) {
    if (draft) updateTicket(draft);
    navigate(path);
  }
  ...
  function handleCreateSubtask() {
    ...
    setTickets((prev) => {
      ...
      const subtask: Ticket = { id: newId, title: subtaskTitle.trim(), status: 'planned', issueType: 'subtask', parentId, sprintId: draft!.sprintId };
      return [
  ...
  const subtasks = tickets.filter((t) => t.parentId === draft.id);
  const breadcrumb = buildBreadcrumb(ticket, tickets);

  return (
    <div className="td-page">
      {/* Breadcrumb */}
      <nav className="td-breadcrumb" aria-label="Breadcrumb">
        <Link to="/board" className="td-breadcrumb__item">Spaces</Link>
        <span className="td-breadcrumb__sep">/</span>
        <Link to="/board" className="td-breadcrumb__item">
          <span className="td-breadcrumb__space-dot" style={{ background: currentSpace.color }} />
          {currentSpace.name}
        </Link>
            ...
            <span key={item.id} className="td-breadcrumb__segment">
              ...
              {isLast ? (
                ...
                <Link
                  ...
                  onClick={() => { if (draft) updateTicket(draft); }}
                >
                  <IssueTypeIcon type={item.issueType} />
                  {item.id}
                </Link>
      ...
      <div className="td-content">
        {/* Left panel */}
        <div className="td-content__left">
          {/* Title */}
          <div className="td-content__title-area">
            {editingTitle ? (
              <input
                ref={titleInputRef}
              ...
              <h1 className="td-content__title" onClick={() => setEditingTitle(true)} title="Click to edit">
                {draft.title || <span className="ticket-detail__placeholder">Add title…</span>}
              </h1>
            )}
            <span className="ticket-detail__status-badge" style={{ background: statusColor + '22', color: statusColor, borderColor: statusColor + '55' }}>
            ...
            {editingDesc ? (
              <div className="ticket-detail__desc-editor">
                ...
                <div className="ticket-detail__desc-actions">
                  <button type="button" className="ticket-detail__btn ticket-detail__btn--primary" onClick={() => { setEditingDesc(false); handleSave(); }}>Save</button>
            ...
            <div className="ticket-detail__subtasks">
              {subtasks.map((sub) => (
                <button
                  ...
                  onClick={() => saveAndNavigate(`/ticket/${sub.id}`)}
                >
                  ...
                  <span className="ticket-detail__subtask-status-dot" style={{ background: STATUS_COLORS[sub.status] }} />
                </button>
              ))}
              {addingSubtask ? (
          ...
          <div className="ticket-detail__section">
            ...
            <div className="ticket-detail__comments">
              {(draft.comments ?? []).length === 0 && <p className="ticket-detail__no-comments">No comments yet.</p>}
        ...
        {/* Right panel */}
        <aside className="td-content__right">
          ...
          <DetailRow label="Issue Type">
            <select
              ...
              onChange={(e) => patch('issueType', e.target.value as IssueType)}
            >
          ...
          <DetailRow label="Assignee">
            <select className="ticket-detail__select" value={draft.assignee ?? ''} onChange={(e) => patch('assignee', e.target.value || undefined)}>
          ...
          <DetailRow label="Reporter">
            <select className="ticket-detail__select" value={draft.reporter ?? ''} onChange={(e) => patch('reporter', e.target.value || undefined)}>
          ...
          <DetailRow label="Labels">
            <LabelSelect selected={draft.labels ?? []} onChange={(v) => patch('labels', v.length ? v : undefined)} />
          </DetailRow>
          ...
          <DetailRow label="Sprint">
            <select
              ...
              onChange={(e) => {
                ...
                setDraft((prev) => prev ? { ...prev, sprintId: id, sprint: name } : prev);
              }}
            >
          ...
          <DetailRow label="Story Points">
            <input className="ticket-detail__input" type="number" placeholder="—" min={0} value={draft.storyPoints ?? ''} onChange={(e) => patch('storyPoints', e.target.value ? Number(e.target.value) : undefined)} />
          </DetailRow>

          <DetailRow label="Start Date">
            <input className="ticket-detail__input" type="date" value={toInputDate(draft.startDate)} onChange={(e) => patch('startDate', e.target.value ? formatDisplayDate(e.target.value) : undefined)} />
          </DetailRow>

          <DetailRow label="Due Date">
            <input className="ticket-detail__input" type="date" value={toInputDate(draft.dueDate)} onChange={(e) => patch('dueDate', e.target.value ? formatDisplayDate(e.target.value) : undefined)} />
          </DetailRow>

          <div style={{ paddingTop: '0.75rem' }}>
            <button type="button" className="ticket-detail__btn ticket-detail__btn--primary" style={{ width: '100%' }} onClick={handleSave}>
              Save Changes
            </button>
          </div>
        </aside>
